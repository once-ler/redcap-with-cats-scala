...
private object LimsSpecimenRemoteSQL {
  ...
  val listFragment = fr"""
    ...
    ('LV ParticipantID:'||s_samplefamily.participantid||';EVENT:'||(select eventlabel from s_participantevent where s_participanteventid = s_samplefamily.participanteventid)||';SURGICAL_PATHOLOGY_NUMBER:'||s_samplefamily.u_nyusurgicalpathid||';ANATOMIC_SITE:'||(select tissuedesc from s_tissue where s_tissueid = s_samplefamily.tissueid)||';TISSUE_STATUS:'||s_sample.u_tissuestatus||';QUANTITY:'||(select trackitem.qtycurrent from trackitem where trackitem.linkkeyid1 = s_sample.s_sampleid)||';UNIT:'||(select trackitem.qtyunits from trackitem where trackitem.linkkeyid1 = s_sample.s_sampleid)||';STORAGE_STATUS:'||s_sample.storagestatus||';SAMPLE_TYPE:'||s_sample.sampletypeid||';LOCATION:'||(select min(labelpath) from storageunit,trackitem where trackitem.linksdcid ='Sample' and trackitem.linkkeyid1 = s_sample.s_sampleid and currentstorageunitid is not null and currentstorageunitid = storageunitid)) samplevalue,
